...
export default function MarketChat() {
  ...
  const otherUserId = searchParams.get("other") || "";   // <-- other user's ID
  ...
  const fetchItemInfo = async () => {
    try {
      const res = await api.get(`/marketplace/items/${itemId}`);
      setItem(res.data);
    } catch (err) {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const res = await api.get(`/marketplace/items/${itemId}/messages`, { params });
  ...
  const handleSend = async () => {
    ...
    setInputText("");
    try {
      await api.post(`/marketplace/items/${itemId}/messages`, { content: text });
      await fetchMessages();
    } catch (err) {
        ...
        >
          {loading ? (
            ...
            messages.map((msg, idx) => {
              ...
              const showAvatar =
                !isMe && (idx === 0 || messages[idx - 1]?.sender_id !== msg.sender_id);
