...
const universities = [
  ...
  { id: 'sun', name: 'Stellenbosch University', short: 'SU', location: 'Stellenbosch' },
  { id: 'uct', name: 'University of Cape Town', short: 'UCT', location: 'Cape Town' },
  { id: 'ufh', name: 'University of Fort Hare', short: 'UFH', location: 'Alice' },
...
const UniversitySelect = () => {
  ...
  const handleSelectUniversity = async (university) => {
    // Always save the choice to localStorage (used by RootRoute and other components)
    localStorage.setItem('selectedUniversity', JSON.stringify(university));
    window.dispatchEvent(new Event('universitySelected'));

    if (user) {
      // Already logged in – set the university in the backend and go home
      try {
        await api.post("/university/set", { university_id: university.id });
        navigate("/");
      } catch (err) {
        toast.error(err.response?.data?.detail || "Could not set university");
      }
    } else {
      // Not logged in – go to the login page so they can sign in with their student email
      navigate("/login");
    }
  };
