...
export default function ClubChat() {
  ...
  const fetchClubInfo = async () => {
    try { const res = await api.get(`/clubs/${clubId}`); setClub(res.data); } catch { navigate("/clubs"); }
  };
    ...
    <div className="min-h-screen bg-[#EFEAE2]">
      ...
      <main className="max-w-2xl mx-auto h-[calc(100vh-4rem)] flex flex-col">
        <div className="bg-[#F0F2F5] border-b px-4 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <div><h3 className="font-semibold text-sm">{club?.title || "Group Chat"}</h3><p className="text-xs text-gray-500">{messages.length} messages</p></div>
        </div>
           ...
           messages.map((msg, idx) => {
             ...
             return (
               <div key={msg.message_id} className={`flex ${isMe ? "justify-end" : "justify-start"} items-end gap-1`}>
                 {!isMe && <div className="w-9 h-9 rounded-full overflow-hidden bg-gray-300 flex-shrink-0 mb-0.5">{showAvatar ? (msg.sender_picture ? <img src={msg.sender_picture} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs font-bold">{(msg.sender_name||"?")[0]}</div>) : <div className="w-9 h-9" />}</div>}
