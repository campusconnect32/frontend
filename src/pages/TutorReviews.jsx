...
export default function TutorReviews() {
  ...
  useEffect(() => {
    fetchTutorInfo();
    fetchMyRating();        // load existing rating first
    fetchMessages();
  ...
  const fetchMyRating = async () => {
    try {
      const res = await api.get(`/tutors/${tutorId}/my-rating`);
      setRating(res.data.rating || 0);
    } catch (err) {
      console.error("Failed to load your rating", err);
    }
  };
  ...
  const fetchMessages = async () => {
    try {
      ...
      const all = res.data || [];
      // Only show messages that have actual text (exclude rating‑only entries)
  ...
  const handleSend = async () => {
    ...
    setInputText("");
    try {
      await api.post(`/tutors/${tutorId}/reviews`, { rating: 0, comment: text });
      await fetchMessages();
    } catch (err) {
  ...
  const handleRate = async (star) => {
    setRating(star);           // optimistically update the UI
    setSubmittingRating(true);
    try {
      // Rating only – no comment, will never appear in chat
        ...
        <div className="bg-[#F0F2F5] border-b px-4 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <div>
            <h3 className="font-semibold text-sm text-gray-900">
              {tutor?.title || "Tutor Chat"}
            </h3>
            <p className="text-xs text-gray-500">{messages.length} messages</p>
          </div>
        </div>

        {/* Rating bar – shows the user's current rating and persists */}
        <div className="bg-white border-b px-4 py-2 flex items-center justify-center gap-1 flex-shrink-0">
          <span className="text-xs text-gray-500 mr-2">Rate this tutor:</span>
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              key={star}
        ...
        >
          {loading ? (
            ...
            messages.map((msg, idx) => {
              ...
              const showAvatar =
                !isMe && (idx === 0 || messages[idx - 1]?.user_id !== msg.user_id);
                ...
                >
                  ...
                  <div className="max-w-[75%] group">
                    <div className="relative">
                      {!isMe && (
                        <div
                          className="absolute top-0 -left-[6px] w-2 h-2 bg-white"
                          style={{ clipPath: "polygon(100% 0, 0 0, 100% 100%)" }}
                        />
                      )}
                      {isMe && (
                        <div
                          className="absolute bottom-0 -right-[6px] w-2 h-2 bg-[#005C4B]"
                      ...
                      >
                        {!isMe && (
                          <p className="px-2.5 pt-1.5 pb-0 text-[11px] font-semibold text-[#005C4B]">
                            {msg.user_name}
                          </p>
                        )}
                        <div
                          className={`${
                            !isMe ? "px-2.5 pb-1.5 pt-0" : "px-2.5 py-1.5"
                          } pr-16`}
                        >
                        ...
                        <div
                          className={`absolute bottom-0.5 right-2 flex items-center gap-1 text-[10px] ${
                            isMe ? "text-white/55" : "text-gray-400"
                          }`}
                        >
